...
<form action="<?php echo apply_filters('paypal_donation_plus_url', $url); ?>" method="post"<?php
if (isset($pd_options['new_tab'])) {
        echo ' target="_blank"';
}
?>>
    <div class="paypal-donation-plus">
        <input type="hidden" name="cmd" value="_donations" />
        <input type="hidden" name="bn" value="PayPalDonation_Plus" />
        <input type="hidden" name="business" value="<?php echo apply_filters('paypal_donation_plus_merchant_email', $pd_options['paypal_account']); ?>" />
<?php
        # Criar o botão
        $paypal_btn = '';
        $indent = str_repeat(" ", 8);
        ...
        if ($purpose)
            $paypal_btn .=  apply_filters('paypal_donation_plus_purpose_html', $indent.'<input type="hidden" name="item_name" value="' .esc_attr($purpose). '" />'.PHP_EOL);  // Purpose
        if ($reference)
        ...
        if ($amount){
            if(!is_numeric($amount)){
                wp_die('You got an error! Donation amount must be a numeric value.');
            }
            $paypal_btn .=  $indent.'<input type="hidden" name="amount" value="' . apply_filters( 'paypal_donation_plus_amount', $amount ) . '" />'.PHP_EOL;
        }
